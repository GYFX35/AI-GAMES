...
def create_github_release(token, repo_name, tag_name, release_name, body, asset_path=None):
    """
    Create a new release in the specified GitHub repository.
    """
    g = Github(token)
    repo = g.get_repo(repo_name)
    release = repo.create_git_release(tag=tag_name, name=release_name, message=body)
    print(f"Successfully created release '{release_name}' with tag '{tag_name}'")

    if asset_path:
        release.upload_asset(asset_path)
        print(f"Successfully uploaded asset '{asset_path}' to release '{release_name}'")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--asset-path", help="Path to the asset to upload")
